...
```
## Simple example to add labels to gke associated to a gcp infrastructure
...
resource "duplocloud_infrastructure" "myinfra" {
  ...
  account_id  = var.infra_config["account_id"]
  is_serverless_kubernetes = true
  cluster_ip_cidr = "172.19.0.0/12"
  setting {
    key="K8sVersion"
    value = "1.33.2-gke.1240000"
  }
  setting {
    key="labels"
    value = jsonencode({"infra": "duplo", "test": "universe"})
  }
  azcount = var.infra_config["azcount"]
  subnet_cidr = var.infra_config["subnet_cidr"]
}
