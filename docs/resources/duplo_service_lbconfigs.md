...
// Example: GCP load balancer with advanced settings (access logs, Cloud Armor, timeouts)
resource "duplocloud_duplo_service" "gcp_service" {
  tenant_id = duplocloud_tenant.myapp.tenant_id
...
resource "duplocloud_duplo_service_lbconfigs" "gcp_lb" {
  ...
  lbconfigs {
    lb_type          = 1 # ALB equivalent for GKE ingress
    protocol         = "HTTP"
    port             = "80"
    external_port    = 80
    health_check_url = "/"
