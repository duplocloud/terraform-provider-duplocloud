...
- `make_headless` (Boolean) Removes every instance under the secondary cluster while retaining the cluster itself, including any reader added with `duplocloud_rds_read_replica`. Read replicas can only be added to the secondary cluster while this is `false`. Valid during update. Defaults to `false`.
